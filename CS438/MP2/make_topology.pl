...
# flush interfaces
my @interfaces = (`ifconfig | grep eth0: | awk -F' ' '{ print \$1 }'`);
foreach my $interface (@interfaces) {
    $interface = substr $interface, 0, -1;
...
`sudo iptables -A OUTPUT -s 10.1.1.0/24 -j DROP`;
`sudo iptables -A OUTPUT -s 10.0.0.10 -j DROP`;
